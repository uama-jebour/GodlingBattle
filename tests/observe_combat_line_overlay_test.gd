...
func _run() -> void:
	...
	session_state.last_timeline = [
		{
			"tick": 0,
			"entities": [
				{"entity_id": "hero_1", "display_name": "英雄", "side": "hero", "alive": true, "hp": 100.0, "max_hp": 100.0, "attack_range": 700.0, "position": Vector2(200, 240)},
				{"entity_id": "enemy_1", "display_name": "敌方", "side": "enemy", "alive": true, "hp": 100.0, "max_hp": 100.0, "position": Vector2(760, 240)}
			]
		},
		{
			"tick": 1,
			"entities": [
				...
				{"entity_id": "enemy_1", "display_name": "敌方", "side": "enemy", "alive": true, "hp": 78.0, "max_hp": 100.0, "position": Vector2(720, 240)}
			]
		}
	]
	...
	if not screen.has_method("get_combat_line_count"):
		...
		_failures.append("combat lines should be visible at frame 1")
	...
	if not screen.has_method("get_attack_line_count"):
		...
		_failures.append("attack line should be visible when hp drops")

	if not screen.has_method("get_strategy_line_count"):
		_failures.append("missing get_strategy_line_count")
	elif int(screen.call("get_strategy_line_count")) <= 0:
		_failures.append("strategy cast should generate strategy target lines")
